...
const Navbar = ({ handleOrderPopup, handleLoginPopup }) => {
  ...
  // Đóng markdown (dropdown) khi nhấn ra ngoài khung
  useEffect(() => {
    const handleClickOutside = (event) => {
      // Nếu đang mở dropdown user
      if (showOption && dropdownRef.current && !dropdownRef.current.contains(event.target)) {
        setShowOption(false);
      }
      // Nếu đang mở dropdown thông báo (desktop)
      if (showNoti && notiRef.current && !notiRef.current.contains(event.target)) {
        setShowNoti(false);
      }
      // Ẩn luôn mobile menu nếu click ra ngoài
      if (showMobileMenu) {
        const mobileMenuBtn = document.getElementById("mobile-menu-btn");
        // Nếu click không phải vào nút menu hoặc menu
        if (
          mobileMenuBtn &&
          !mobileMenuBtn.contains(event.target) &&
          !event.target.closest(".absolute.right-4.top-20")
        ) {
    ...
    document.addEventListener("mousedown", handleClickOutside);
    return () => document.removeEventListener("mousedown", handleClickOutside);
  }, [showOption, showMobileMenu, showNoti]);
          ...
          <div className="flex items-center gap-2 md:gap-8">
            ...
            {Cookies.get('authToken') ? (
              <>
                {/* Xu */}
                {user && (
                  <div className="hidden md:flex items-center gap-1 text-green-600 dark:text-green-400 font-bold text-base ml-2">
                ...
                <div className="relative ml-2 hidden md:block" ref={notiRef}>
                  ...
                  {showNoti && (
                    <div
                      ref={notiRef}
                      className="absolute right-0 mt-2 w-96 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-xl z-50 transform transition-all duration-200 ease-out"
                    >
                            ...
                            >
                              <div className="flex items-start gap-3">
                                ...
                                <button
                                  ...
                                  className="p-1 text-gray-400 hover:text-red-500 dark:text-gray-500 dark:hover:text-red-400 transition-all duration-200"
                ...
                {/* Avatar + Option */}
                <div className="relative flex items-center hidden md:flex" style={{ marginTop: '2px' }}>
                ...
                {/* Mobile: Xu, Avatar vẫn giữ nguyên như cũ */}
                <div className="flex md:hidden items-center gap-1 text-green-600 dark:text-green-400 font-bold text-base ml-2">
                  <span className="bg-yellow-400 text-white rounded-full px-2 py-0.5 text-xs font-bold">x</span>
                  {user.balance?.toLocaleString() || 0} Xu
                </div>
                <div className="relative flex md:hidden items-center" style={{ marginTop: '2px' }}>
                  <button onClick={() => setShowOption(!showOption)} className="w-14 h-14 ml-2 flex items-center justify-center">
                    <img
                      src={generateAvatar(user.full_name || user.name)}
                      alt="Avatar"
              ...
              {showMobileMenu && (
                ...
                >
                  <ul className="flex flex-col font-semibold text-base">
                    ...
                    {/* Thêm thông báo trên mobile */}
                    {Cookies.get('authToken') && (
                      <li>
                        <button
                          className="flex items-center gap-2 w-full py-2 px-4 hover:bg-primary/10"
                          onClick={() => {
                            setShowNoti(!showNoti);
                            // Không đóng menu để có thể xem thông báo ngay trên mobile
                          }}
                        >
                        ...
                        {/* Hiển thị dropdown thông báo ngay trong menu mobile */}
                        {showNoti && (
                          <div
                            ref={notiRef}
                            className="mt-2 w-full bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-xl z-50"
                          >
                            <div className="p-4 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center">
                              <h3 className="text-lg font-semibold text-gray-900 dark:text-white">Notifications</h3>
                              <div className="flex gap-2">
